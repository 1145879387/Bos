    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#btnEp").click(function () {
                //	先判断输入的检验是否合法
                //	必须要有from输入框，否则无法进行检验
                var form = $("#editpassword").form("validate");
                if (form) {
                    //    还需要两次密码输入检验输入是否一致
                    var val = $("#txtNewPass").val();
                    var val2 = $("#txtRePass").val();
                    if (val == val2) {
                        // alert("asdasdas")
                        $.post("Useraction_editpassword", {"password": val2}, function (data) {
                            if (data == '1') {
                                //    修改成功，关闭窗口
                                $("#editPwdWindow").window("close");
                            } else {
                        ...
                        $.messager.alert("提示信息", "两次密码输入不一致", "waring");
                    }
                }
            });
        });
...
<div data-options="region:'north',border:false"
    ...
    <div id="sessionInfoDiv"
        ...
        [<strong>${login.username}</strong>]，欢迎你！
        <%--[<strong><s:property value="#usernumber.username"/></strong>]，欢迎你！--%>
    </div>
