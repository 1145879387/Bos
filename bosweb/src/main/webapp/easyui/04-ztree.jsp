            ...
            <script type="text/javascript">
                $(function () {
                    ...
                    var setting2 = {
                        ...
                        //callback并不是ajax的参数，而是ztree框架里的回调函数，里面绑定了点击事件用于点击
                        callback: {
                            //当你点击页面节点的时候。，treenode会自动的读取json，而json数据是由ajax获得的
                            onClick: function (event, treeId, treeNode) {
                                if (treeNode.page != null) {
                                    //判断选项卡是否存在
                                    var tabs = $("#mytabs").tabs("exists", treeNode.name);
                                    if (tabs) {
                                        //注意，他是按照title来进行跳转判定的
                    ...
                    //构造节点数据
                    var url1 = "${pageContext.request.contextPath}/json/menu.json";
                    $.post(url1, {}, function (data) {
                        //调用API初始化ztree
                        $.fn.zTree.init($("#ztree20"), setting2, data);
                    }, "json");
                });
            </script>
        </div>
